...
**Ни один из этих сервисов не предоставляет эндпоинта для получения фактического потребления или списаний** — имеется в виду потребление на уровне облачного биллинга в целом (в т.ч. вне Kubernetes: VM, Object Storage, Managed Service for Kubernetes и др.). API позволяет узнать баланс аккаунта, тарифы SKU, список сервисов и бюджеты, но нельзя запросить «сколько потрачено за период X на сервис Y».

...
Помимо чтения метрик из VictoriaMetrics (Prometheus API), OpenCost ведёт **внутренний кэш агрегированных данных стоимости** — Collector DataSource. Это не TSDB и не VictoriaMetrics: кэш хранится **внутри пода OpenCost** через Storage API (по умолчанию — локальная файловая система контейнера или память). При перезапуске пода кэш теряется, если не настроено внешнее хранилище (S3/GCS и т.п.).

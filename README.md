...
**Проверка egress и LB по детализации.** Чтобы в детализации биллинга появились ненулевые строки по исходящему и входящему трафику и Load Balancer (и можно было сверить `zoneNetworkEgress`, `regionNetworkEgress`, `internetNetworkEgress`, `loadBalancer`), в репозитории добавлены два манифеста. **`k8s/stream-server.yaml`** — HTTP-сервис с бесконечной отдачей по адресу http://stream.apatsev.org.ru/stream: долгая загрузка с ноутбука (`curl -o /dev/null http://stream.apatsev.org.ru/stream`) создаёт **egress**-трафик; сервис идёт через тот же Ingress/LB, поэтому в биллинге видны и трафик (VPC), и NLB. **`k8s/ingress-download-cronjob.yaml`** — CronJob, который периодически (по умолчанию каждые 30 минут) скачивает тестовый файл из интернета в кластер в `/dev/null`, создавая **входящий** трафик («Входящий трафик из интернета» в детализации). После экспорта детализации в CSV по этим строкам можно проверить или подставить цены в custom-pricing-configmap (в т.ч. через `scripts/fetch_yandex_sku_prices.py --csv ...`).

...
Ниже приведены ссылки на шесть дашбордов; в разделе [Метрики, которые экспортирует OpenCost](#метрики-которые-экспортирует-opencost) для каждого дашборда — скриншот и краткое описание.

| Файл | GitHub | Описание |
|------|--------|----------|
| **opencost-overview.json** | [ссылка](https://github.com/patsevanton/opencost-yandex-cloud/blob/main/grafana-dashboards/opencost-overview.json) | OpenCost Overview |
| **opencost-namespace.json** | [ссылка](https://github.com/patsevanton/opencost-yandex-cloud/blob/main/grafana-dashboards/opencost-namespace.json) | OpenCost Namespace |
| **opencost-cost-reporter-basic-overview.json** | [ссылка](https://github.com/patsevanton/opencost-yandex-cloud/blob/main/grafana-dashboards/opencost-cost-reporter-basic-overview.json) | OpenCost Cost reporter — базовый обзор |
| **opencost-cost-reporter-detailed-overview.json** | [ссылка](https://github.com/patsevanton/opencost-yandex-cloud/blob/main/grafana-dashboards/opencost-cost-reporter-detailed-overview.json) | OpenCost Cost reporter — детальный обзор |
| **opencost-network-lb.json** | [ссылка](https://github.com/patsevanton/opencost-yandex-cloud/blob/main/grafana-dashboards/opencost-network-lb.json) | OpenCost Network & Load Balancer |
| **opencost-health.json** | [ссылка](https://github.com/patsevanton/opencost-yandex-cloud/blob/main/grafana-dashboards/opencost-health.json) | OpenCost Health |
...
OpenCost отдаёт метрики на порту **9003** (`/metrics`). Ниже для каждого дашборда — скриншот и краткое описание. При кастомных ценах в рублях (`currency: RUB`) значения cost-метрик — в **₽/час** (или ₽/GiB для egress), а не в USD.

### OpenCost Overview

![OpenCost Overview](images/grafana-dashboards/opencost-overview.png)

Обзор стоимости кластера: почасовая, дневная и месячная стоимость; разбивка по namespace и типу инстансов; стоимость нод, PV и Load Balancer; блоки GPU и Spot.

### OpenCost Namespace

![OpenCost Namespace](images/grafana-dashboards/opencost-namespace.png)

Детализация по namespace: почасовая/дневная/месячная стоимость, CPU и RAM по namespace, стоимость по ресурсам, подам и контейнерам, сводка по PV, аллокации GPU и PVC.
...
Базовый отчёт: средняя дневная стоимость, почасовая стоимость кластера, оценка месячных расходов; топ-20 namespace, контейнеров и подов; относительная цена и стандартное отклонение.
...
Детальный отчёт: топ по namespace и контейнерам, почасовая стоимость, текущая цена за месяц/день/час, стоимость по приложениям, средние и кластерные показатели, PVC.

### OpenCost Network & Load Balancer

![OpenCost Network & Load Balancer](images/grafana-dashboards/opencost-network-lb.png)

Сетевые расходы: egress по зоне, региону и в интернет (₽/GiB), NAT Gateway (egress/ingress), почасовая и месячная стоимость Load Balancer по namespace и сервисам. При использовании NLB за ingress-nginx OpenCost может не обнаруживать LB — метрика бывает пустой.

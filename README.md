...
**Сохранение кэша при перезапуске пода:** в `opencost-values.yaml` включите `opencost.exporter.persistence` (см. закомментированный блок в values). Чарт создаст PVC и примонтирует том к поду; кэш будет храниться на PV. В Yandex Cloud можно использовать StorageClass `yc-network-hdd` или `yc-network-ssd` (или оставить пусто для default).

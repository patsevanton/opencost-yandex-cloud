...
## Collector DataSource OpenCost

Помимо чтения метрик из VictoriaMetrics (Prometheus API), OpenCost ведёт **внутренний кэш агрегированных данных стоимости** — Collector DataSource. Это не TSDB и не VictoriaMetrics: кэш хранится **внутри пода OpenCost** через Storage API (по умолчанию — локальная файловая система контейнера или память). При перезапуске пода кэш теряется, если не включена персистентность: **Persistent Volume (PV/PVC)** через настройки Helm (`opencost.exporter.persistence`) или внешнее хранилище (S3/GCS).

В `opencost-values.yaml` срок хранения этого кэша задаётся параметрами:

| Параметр     | Описание |
|-------------|----------|
| `retention1d` | Число интервалов по 1 дню — сколько дней хранить данные с дневным разрешением |
| `retention1h` | Число интервалов по 1 часу — сколько часов хранить данные с часовым разрешением |

Они влияют только на внутренний кэш OpenCost (аллокации, агрегаты за прошлые периоды), а не на срок хранения метрик в VictoriaMetrics.

**Сохранение кэша при перезапуске пода:** в `opencost-values.yaml` включите `opencost.exporter.persistence` (см. закомментированный блок в values). Чарт создаст PVC и примонтирует том к поду; кэш будет храниться на PV.

## Prometheus Operator CRD

Используем Prometheus Operator CRD, чтобы не писать конфигурации vmscrape (scrape для VictoriaMetrics) вручную. CRD нужен для ServiceMonitor OpenCost: чарт OpenCost создаёт ServiceMonitor в namespace `opencost`, и vmagent (VictoriaMetrics) по нему автоматически скрейпит метрики OpenCost.
...
Все цены в ConfigMap заданы в **рублях (₽)**. В `custom-pricing-configmap.yaml` указано `currency: "RUB"` — часть UI и API OpenCost может отображать эту валюту. Если в интерфейсе по-прежнему показывается «Currency USD», это лишь подпись по умолчанию: **числовые значения у нас в рублях**. Глобально OpenCost по умолчанию использует USD; для нашего custom pricing мы явно задаём RUB, чтобы по возможности везде отображалась правильная валюта.
...
Опции поддерживаются **Helm-чартом** OpenCost (в `deployment.yaml` прокидываются в под как env) и **приложением** OpenCost (используются при эмиссии метрик на `/metrics`). В [issue #3146](https://github.com/opencost/opencost/issues/3146) фигурирует OpenCost 1.114.0 с этой настройкой — при текущей версии чарта (2.5.10) опция применима.

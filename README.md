...
Установите **первым**: ресурс **ServiceMonitor** — это CRD из Prometheus Operator (`monitoring.coreos.com`). Он нужен для ServiceMonitor OpenCost: чарт OpenCost создаёт ServiceMonitor в namespace `opencost`, чтобы vmagent (VictoriaMetrics) скрейпил метрики OpenCost. VictoriaMetrics Stack при установке обычно уже ставит эти CRD. Если вы ставите OpenCost в кластер без VMKS или CRD в кластере нет, установите только CRD:

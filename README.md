...
4. **Проверка** — после применения ConfigMap перезапустите под OpenCost (или дождитесь обновления конфига). В Prometheus/VictoriaMetrics проверьте метрики `node_cpu_hourly_cost`, `node_ram_hourly_cost` — в них должны быть ваши ставки.

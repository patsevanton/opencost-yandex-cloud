...
**Проверка egress и LB по детализации.** Чтобы в детализации биллинга появились ненулевые строки по исходящему трафику и Load Balancer (и можно было сверить `zoneNetworkEgress`, `regionNetworkEgress`, `internetNetworkEgress`, `loadBalancer`), в репозитории добавлен манифест `k8s/stream-server.yaml`: HTTP-сервис с бесконечной отдачей по адресу http://stream.apatsev.org.ru/stream. Долгая загрузка с ноутбука (`curl -o /dev/null http://stream.apatsev.org.ru/stream`) создаёт egress-трафик; сам сервис доступен через тот же Ingress/LB, что и остальные приложения, поэтому в биллинге появляются и трафик (VPC), и нагрузка на NLB. После экспорта детализации в CSV по ним можно проверить или подставить цены в custom-pricing-configmap (в т.ч. через `scripts/fetch_yandex_sku_prices.py --csv ...`).
